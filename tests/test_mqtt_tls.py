import sys
import unittest
from unittest.mock import MagicMock, patch
import os

# Mock dependencies
sys.modules['psutil'] = MagicMock()
sys.modules['paho'] = MagicMock()
sys.modules['paho.mqtt'] = MagicMock()
sys.modules['paho.mqtt.client'] = MagicMock()
sys.modules['pynvml'] = MagicMock()

# Import MQTTClient
sys.path.append(os.path.join(os.getcwd(), 'system_monitor'))
from mqtt_client import MQTTClient

class TestMQTTClientTLS(unittest.TestCase):
    def test_tls_configuration(self):
        # Test case: TLS Enabled
        tls_config = {
            'ca_certs': '/path/to/ca.crt',
            'certfile': '/path/to/client.crt',
            'keyfile': '/path/to/client.key',
            'insecure': False
        }

        client = MQTTClient("localhost", 8883, None, None, "Test Device", tls_config)

        # Verify tls_set called with correct args
        client.client.tls_set.assert_called_with(
            ca_certs='/path/to/ca.crt',
            certfile='/path/to/client.crt',
            keyfile='/path/to/client.key'
        )

        # Verify tls_insecure_set NOT called (since insecure is False/default)
        client.client.tls_insecure_set.assert_not_called()

    def test_tls_insecure(self):
        # Test case: TLS Enabled + Insecure
        tls_config = {
            'ca_certs': '/path/to/ca.crt',
            'insecure': True
        }

        client = MQTTClient("localhost", 8883, None, None, "Test Device", tls_config)

        client.client.tls_set.assert_called_with(
            ca_certs='/path/to/ca.crt',
            certfile=None,
            keyfile=None
        )

        # Verify tls_insecure_set called
        client.client.tls_insecure_set.assert_called_with(True)

    def test_no_tls(self):
        # Test case: No TLS config
        client = MQTTClient("localhost", 1883, None, None, "Test Device", None)

        # Verify tls_set NOT called
        client.client.tls_set.assert_not_called()

if __name__ == '__main__':
    unittest.main()
